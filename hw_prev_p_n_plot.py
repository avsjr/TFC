...
plt.plot(forecast_data['Ano'], forecast_data['Indice Aprovação'], marker='o', label='Indice Aprovação (P)')
plt.plot(forecast_data['Ano'], forecast_data['SAEBE'], marker='o', label='SAEBE (N)')
plt.plot(ano + [2023, 2025], ideb, marker='o', label='IDEB', linestyle='--', color='red')
plt.xlabel('Ano')
plt.ylabel('Valores')
plt.title('Previsões para Indice Aprovação (P), SAEBE (N) e IDEB em 2023 e 2025')
plt.legend()
plt.grid(True)
plt.show()
